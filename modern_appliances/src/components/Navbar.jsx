...
const Navbar = () => {
  ...
  // is_admin comes directly from the login response stored in Redux
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-80 text-center">
            ...
            <div className="flex gap-3">
              <button onClick={handleLogout} className="flex-1 py-2.5 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition">
                Yes, Sign Out
              </button>
              <button onClick={() => setShowLogout(false)} className="flex-1 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition">
                Cancel
              </button>
            </div>
          </div>
      ...
      <nav className="bg-black border-b border-gray-800 px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="hidden md:flex items-center gap-5">
          <Link to="/shop" className="text-gray-300 hover:text-amber-400 text-sm font-medium transition">Shop</Link>
          ...
          {isAuthenticated && user ? (
            <>
              ...
              <Link to="/profile" className="flex items-center gap-2 group">
                ...
                <span className="text-gray-300 group-hover:text-amber-400 text-sm transition">{displayName}</span>
              </Link>

              {/* Admin button — only shown if is_admin is true */}
              {isAdmin && (
                <Link
                  to="/admin-dashboard"
              ...
              <button onClick={() => setShowLogout(true)} className="text-gray-500 hover:text-white text-sm transition">
                Sign Out
              </button>
            </>
          ) : (
            <>
              ...
              <Link to="/register" className="bg-amber-400 text-black px-4 py-1.5 rounded-full text-sm font-black hover:bg-amber-300 transition">
                Register
              </Link>
            </>
          )}
      ...
      {menuOpen && (
        <div className="md:hidden bg-gray-950 border-b border-gray-800 px-6 py-5 space-y-4 z-30">
          <Link to="/shop" onClick={() => setMenuOpen(false)} className="block text-gray-300 hover:text-amber-400 text-sm font-medium">Shop</Link>
          ...
          {isAuthenticated ? (
            <>
              ...
              <button onClick={() => { setMenuOpen(false); setShowLogout(true); }} className="block text-gray-500 hover:text-white text-sm">
                Sign Out
              </button>
            </>
          ) : (
